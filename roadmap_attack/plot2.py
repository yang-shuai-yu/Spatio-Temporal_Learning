...
def get_gps_from_segment(selected_labels, selected_outputs, road_segments, rel_table):
    ...
    for i in range(len(selected_labels)):
        ...
        # gps sequence sorting
        label_gps = pointSort(label_gps); output_gps = pointSort(output_gps)
...
def plot_on_map(gps_list, color, m):
    ...
    folium.PolyLine(gps_list[:, ::-1], color=color, weight=10, opacity=0.4).add_to(m)

    return m

def pointSort(points):
    # sort points just like bfs
    # points: list of [lon, lat]
    # get the distance matrix between all points
    ...
    # print(dist_matrix)
    # get the start point, bottom-left point
    start_idx = np.argmin(points[:, 0] + points[:, 1])
    # print(start_idx)
    # get the order of the points
    order = [start_idx]
    for i in range(n-1):
        dist_matrix[:, start_idx] = np.inf
        next_idx = np.argmin(dist_matrix[start_idx])
...
def test(gps_lists, outputs, labels, geo_file, rel_table, args):
    ...
    for i in range(len(selected_gps)):
        ...
        m.save('results/{}_{}_{}.html'.format(args.emb_model, args.emb_size, i))
        print('results/{}_{}_{}.html'.format(args.emb_model, args.emb_size, i))
...
if __name__ == '__main__':
    '''
        Usage:
            python plot2.py --emb_name start_256_test --model_path models/start256_ --emb_model start --emb_size 256
    '''
