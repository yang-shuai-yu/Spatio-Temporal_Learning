...
class WideAndDeep(nn.Module):
    def __init__(self, embedding_dim=128, hidden_dim=256):
        ...
        self.duration_embedding = nn.Embedding(5146, hidden_dim)
        self.hop_embedding = nn.Embedding(218, hidden_dim)
        self.time_embedding = nn.Embedding(546, hidden_dim)
        self.usrid_embedding = nn.Embedding(905, hidden_dim)
        self.deep_fc1 = nn.Linear(hidden_dim*4, embedding_dim)
        self.deep_fc2 = nn.Linear(embedding_dim, embedding_dim)

    def forward(self, attr):
        # Continuous attributes
        continuous_attrs = attr[:, 1:3]  # speed, distance

        # Categorical attributes
        # depature, sid, eid = attr[:, 0].long(
        # ), attr[:, 6].long(), attr[:, 7].long()
        duration, hop, time, usrid = attr[:, 0].long(
        ), attr[:, 3].long(), attr[:, 4].long(), attr[:, 5].long()
        ...
        hop_embed = self.hop_embedding(hop)
        time_embed = self.time_embedding(time)
        usrid_embed = self.usrid_embedding(usrid)
        categorical_embed = torch.cat(
            (duration_embed, hop_embed, time_embed, usrid_embed), dim=1)
...
if __name__ == '__main__':
    ...
    t = torch.randn(10)
    duration = torch.zeros(10)
    # avg_dis = torch.zeros(10)
    # avg_speed = torch.zeros(10)
    # total_dis = torch.zeros(10)
    # total_time = torch.zeros(10)
    # total_len = torch.zeros(10)
    speed = torch.zeros(10)
    lenth = torch.zeros(10)
    hop = torch.zeros(10)
    time = torch.zeros(10)
    id = torch.zeros(10)
    # attr = torch.stack(
    #     [duration, total_dis, total_time, total_len, avg_dis, avg_speed, hop, time], dim=1)
    attr = torch.stack(
        [duration, speed, lenth, hop, time, id], dim = 1)
    unet = Guide_UNet(config)
    x = torch.randn(10, 2, 400)    # in my attempt, it is 400
