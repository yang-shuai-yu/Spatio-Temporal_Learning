...
def main(config, logger, exp_dir):
    ...
    # print(unet)
    traj = np.load('./mydata/gen_data/detoured_train_gps_len400_norm.npy',
                   allow_pickle=True)
    print(traj.shape)
    print(traj[0].shape)
    traj = traj[:, :, :2]     # num, lenth, 2
    head = np.load('./mydata/gen_data/detoured_train_attrib.npy',
                   allow_pickle=True)
    head = head[:, :5]    # num, 5, delete the usrid
