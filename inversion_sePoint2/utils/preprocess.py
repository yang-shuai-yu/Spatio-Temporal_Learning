...
def prepare_data(data_path, data_name, emb_dim):
    ...
    train_trajs = rtrain_trajs[:10000]; train_embs = rtrain_embs[:10000]
...
def get_Normalization(data_path):
    ...
    for path in sub_path:
        ...
        for i, traj in enumerate(grid_seqs):    # len, lon, lat
            traj = np.array(traj)
            traj[:, 0] = (traj[:, 0] - porto_lon_range[0])/(porto_lon_range[1] - porto_lon_range[0])
            traj[:, 1] = (traj[:, 1] - porto_lat_range[0])/(porto_lat_range[1] - porto_lat_range[0])
...
if __name__ == '__main__':
    # 做归一化，不要搞grid了
    os.chdir("/home/yangshuaiyu6791/RepresentAttack/inversion_sePoint2/")    # change the current directory to inversion_sePoint
