...
class TrajDataset(Dataset):
    ...
    def load_data(self):
        ...
        if self.mode == 'train':
            ...
            grid_path = os.path.join(path, 'train', 'train_norm')
        elif self.mode == 'test':
            emb_path = os.path.join(path, 'test', 'test_data')
            grid_path = os.path.join(path, 'test', 'test_norm')
        elif self.mode == 'val':
            emb_path = os.path.join(path, 'val', 'val_data')
            grid_path = os.path.join(path, 'val', 'val_norm')
        else:
    ...
    def __getitem__(self, idx):
        ...
        start_norm = [self.grid[idx][0][0], self.grid[idx][0][1]]    # because every grid is [x, y, cell]
        end_norm = [self.grid[idx][grid_len-1][0], self.grid[idx][grid_len-1][1]]
        se_norms = torch.Tensor([start_norm, end_norm]).float()    # float for loss bachward
        return emb, se_norms